...
async fn migrate_from_nutshell_into(cdk_db_url: &str, nutshell_db_url: &str) -> Result<(), Error> {
    ...
    // Keyset reads go through a transaction, which holds the global keyset
    // advisory lock.
    let existing_keyset_infos = {
        let mut key_tx = MintKeysDatabase::begin_transaction(&db).await?;
        let infos = key_tx.get_keyset_infos().await?;
        key_tx.rollback().await?;
        infos
    };
