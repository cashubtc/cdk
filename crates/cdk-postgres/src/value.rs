...
mod test {
    ...
    fn encode_unsigned(value: u64, ty: &types::Type) -> Result<(), ()> {
        let mut out = types::private::BytesMut::new();
        let unchecked = PgValue::Unsigned(value)
            .to_sql(ty, &mut out)
            .map_err(|_| ());
        let mut out = types::private::BytesMut::new();
        let checked = PgValue::Unsigned(value)
            .to_sql_checked(ty, &mut out)
            .map_err(|_| ());
    ...
    #[test]
    fn unsigned_accepts_values_that_fit() {
        let largest = u64::try_from(i64::MAX).expect("i64::MAX is not negative");
        assert!(encode_unsigned(largest, &types::Type::INT8).is_ok());
        assert!(encode_unsigned(u64::from(u32::MAX) / 2, &types::Type::INT4).is_ok());
    }

    #[test]
    fn unsigned_rejects_values_that_do_not_fit() {
        let past_i64 = u64::try_from(i64::MAX).expect("i64::MAX is not negative") + 1;
        assert!(encode_unsigned(past_i64, &types::Type::INT8).is_err());
        assert!(encode_unsigned(u64::from(u32::MAX), &types::Type::INT4).is_err());
    }
