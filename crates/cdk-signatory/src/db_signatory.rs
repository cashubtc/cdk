...
impl DbSignatory {
    ...
    /// rotation transaction where the reloaded snapshot is authoritative. An
    /// excluded keyset therefore warns on every tick it stays excluded, the
    /// right cadence for a unit that has silently stopped rotating. Warning
    /// only under the transaction would never fire in the case that matters,
    /// since a stuck keyset that is the only due one never opens one.
        ...
        let mut due: Vec<MintKeySetInfo> = keysets
            ...
            .filter(|info| {
                if info.amounts.is_empty() {
                    tracing::warn!(
                        "Keyset {} for unit {} is active and older than the rotation interval but \
                         carries no amounts, so it cannot be rotated automatically",
                        info.id,
                        info.unit
                    );
                    return false;
...
/// the transaction survives and the units that follow still rotate. Skipping is
/// the only workable answer: a contested path is permanent, since the unit's
/// derivation index advances only when a rotation commits, so failing the sweep
/// over it would stall every other unit for as long as the mint runs.
...
mod test {
    ...
    /// configuration, so no construction check can refuse it. The unit it blocks
    /// can never rotate, and that is permanent: its derivation index only
    /// advances on a commit. The sweep must lose that one unit, not every unit.
    #[tokio::test]
    async fn sweep_skips_the_unit_whose_next_path_a_stale_keyset_holds() {
        let (store, _fail_unit, transactions) = FailUnitDb::new().await;
        let sig = Arc::new(
            DbSignatory::new(
                store.clone(),
        ...
        assert_eq!(
            memory_active_keyset_id(&sig, &CurrencyUnit::Sat),
            Some(sat),
            "sat has nowhere to rotate to and keeps its keyset"
        );
        let rotated_usd =
            memory_active_keyset_id(&sig, &CurrencyUnit::Usd).expect("usd active keyset");
        assert_ne!(rotated_usd, usd, "usd is unaffected by sat's blocked path");
        assert_eq!(
            db_keyset_ids(&sig).await.len(),
            ids_before.len() + 1,
            "the sweep commits usd's replacement and nothing else"
        );
    }
