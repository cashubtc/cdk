...
///
/// Every record is deserialized, and one that cannot be read aborts the whole
/// migration rather than being skipped. That is deliberate: a record that will
/// not parse is corruption or a bug, and converting around it would leave it
/// with no mint id and therefore unreachable forever. Failing here rolls the
/// transaction back, so the database is left exactly as it was and the wallet
/// can be opened by the previous version.
