...
Nostr requests use explicitly listed mints (`--mints`) or configured mints for the
requested unit. Requests are saved automatically; use `check-requests` after
restarting or `cancel-request <payment-id>` to cancel. Offline delivery depends
on relay retention. Requests from the older CLI-only store must be recreated.
