...
impl ChainSource {
    ...
    /// floor. Synchronous so it works in the payjoin `can_broadcast` closure.
    #[cfg_attr(not(feature = "bitcoin-rpc"), allow(unused_variables))]
    pub(crate) fn accepts_broadcast(&self, tx: &Transaction) -> Result<Option<bool>, Error> {
        match self {
            ...
            #[cfg(feature = "bitcoin-rpc")]
            ChainSource::BitcoinRpc(config) => {
                bitcoin_rpc::accepts_broadcast_bitcoin_rpc(config, tx).map(Some)
            }
            #[allow(unreachable_patterns)]
            _ => unreachable!("ChainSource must have at least one feature enabled"),
        }
...
mod tests {
    ...
    #[test]
    fn esplora_has_no_test_broadcast_capability() {
        ...
        assert_eq!(chain.accepts_broadcast(&tx).unwrap(), None);
    }
}
