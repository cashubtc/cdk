...
/// Secondary namespace for finalized receive intent quote id index (quote_id -> Vec<intent_id>)
pub const FINALIZED_RECEIVE_INTENT_QUOTE_ID_NAMESPACE: &str = "finalized_receive_intent_quote_id";
...
mod tests {
    ...
    async fn test_finalize_confirmed_intent_and_cleanup_batch() {
        ...
        let batch = SendBatchRecord {
            ...
            state: SendBatchState::Broadcast {
                ...
                tx_bytes: vec![0x01],
                intent_ids: vec![intent_id_1, intent_id_2],
                fee_sat: 500,
            },
        };
        ...
        let batch_intent_ids = match &batches[0].state {
    ...
    async fn test_pre_broadcast_recovery_reverts_intents() {
        ...
        for state in [
            ...
            let batch_id = Uuid::new_v4();
            let intent_ids = match &state {
                SendBatchState::Built { intent_ids, .. } => intent_ids.clone(),
                SendBatchState::Signed { intent_ids, .. } => intent_ids.clone(),
    ...
    async fn test_post_broadcast_and_orphaned_recovery_storage_shapes() {
        ...
        let batch = SendBatchRecord {
            ...
            state: SendBatchState::Broadcast {
                ...
                intent_ids: vec![broadcast_intent_id],
                fee_sat: 200,
            },
        };
        storage.store_send_batch(&batch).await.expect("store batch");
    ...
    async fn test_recovery_shape_batch_and_intent_can_disagree_on_membership() {
        ...
        let stored_batch = storage
            ...
            .expect("batch exists");
        match stored_batch.state {
            SendBatchState::Broadcast { intent_ids, .. } => {
                assert!(
                    intent_ids.is_empty(),
    ...
    async fn test_recovery_shape_batch_can_have_mixed_intent_states() {
        ...
        let batch = SendBatchRecord {
            ...
            state: SendBatchState::Broadcast {
                ...
                tx_bytes: vec![0x01, 0x02, 0x03],
                intent_ids: vec![batched_intent_id, awaiting_intent_id],
                fee_sat: 400,
            },
        };
        storage.store_send_batch(&batch).await.expect("store batch");
