...
use self::record::{SendBatchRecord, SendBatchState};
...
impl SendBatch<Built> {
    ...
    pub async fn sign(
        ...
        let intent_ids: Vec<Uuid> = self.intents.iter().map(|i| i.intent_id).collect();
...
mod tests {
    ...
    async fn test_built_to_signed_to_broadcast() {
        ...
        let signed_batch = built_batch
            .sign(&storage, tx_bytes.clone(), 500)
            .await
            .expect("sign");
        ...
        let result = signed_batch
            .mark_broadcast(&storage, txid.clone(), tx_bytes.clone(), 500)
            .await
