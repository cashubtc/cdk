        ...
        for batch_record in batches {
            ...
            match batch_record.state {
                SendBatchState::Built {
                    ...
                    let psbt = match bdk_wallet::bitcoin::psbt::Psbt::deserialize(&psbt_bytes) {
                        Ok(psbt) => psbt,
                        Err(err) => {
                            tracing::error!(
                                "Failed to deserialize PSBT for built batch {} during recovery: {}",
                                batch_record.batch_id,
                                err
                            );
                            continue;
...

                    {
                        ...
                        if let Err(err) = wallet_with_db.persist() {
                            tracing::error!(
                                "Failed to persist wallet after cancelling built batch {}: {}",
                                batch_record.batch_id,
                                err
                            );
                            continue;
