...
mod test {
    ...
    #[tokio::test]
    async fn duplicate_auth_proof_insert_rejected() {
        let path = std::env::temp_dir().join(format!(
            "cdk-blind-auth-test-{}.sqlite",
            uuid::Uuid::new_v4()
        ));
        ...
        spend_auth_proof(db.clone(), proof.clone())
            .await
            .expect("first spend");
        let second = spend_auth_proof(db.clone(), proof).await;

        assert!(matches!(second, Err(database::Error::Duplicate)));
        assert_eq!(
