...
mod tests {
    ...
    #[test]
    fn test_valid_signatures_duplicate() {
        let secret_key = SecretKey::generate();
        let pubkey = secret_key.public_key();
        let msg = b"test message";
        let sig = secret_key.sign(msg).unwrap();

        // One signature, one pubkey - OK
        assert_eq!(valid_signatures(msg, &[pubkey], &[sig]).unwrap(), 1);

        // Two identical signatures, one pubkey - Error
        let res = valid_signatures(msg, &[pubkey], &[sig, sig]);
        assert!(res.is_err());
        assert!(matches!(res.unwrap_err(), Error::DuplicateSignature));
    }

    #[test]
    fn test_blinded_message_verify_p2pk_duplicate_sig() {
        let secret_key = SecretKey::generate();
        let pubkey = secret_key.public_key();
        ...
        // Add the same signature again to the witness
        if let Some(Witness::P2PKWitness(p2pk_witness)) = &mut blinded_message.witness {
            let sig = p2pk_witness.signatures[0].clone();
            p2pk_witness.signatures.push(sig);
        }

        // Verification should fail with DuplicateSignature error
        let res = blinded_message.verify_p2pk(&vec![pubkey], 1);
        assert!(res.is_err());
        assert!(
    ...
    #[test]
    fn test_blinded_message_verify_p2pk_duplicate_xonly() {
        let secret_key = SecretKey::generate();
        let pubkey = secret_key.public_key();

        // Create another pubkey with same x-coordinate but different parity
        let mut bytes = pubkey.to_bytes();
        bytes[0] = if bytes[0] == 0x02 { 0x03 } else { 0x02 };
        let pubkey_alt = PublicKey::from_slice(&bytes).unwrap();
        ...
        // Verification should fail if both pubkeys are provided
        let res = blinded_message.verify_p2pk(&vec![pubkey, pubkey_alt], 1);
        assert!(res.is_err());
        assert!(
            matches!(res.unwrap_err(), Error::DuplicateSignature),
            "Expected DuplicateSignature error for same x-only pubkeys"
        );
    }
