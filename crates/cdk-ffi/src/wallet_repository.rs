...
impl WalletRepository {
        ...
        let wallet = match tokio::runtime::Handle::try_current() {
            Ok(handle) => tokio::task::block_in_place(|| {
                handle.block_on(async move {
                    CdkWalletRepository::new(localstore, seed).await
                })
            }),
            Err(_) => {
    ...
    pub fn new_with_proxy(
        ...
        let wallet = match tokio::runtime::Handle::try_current() {
            Ok(handle) => tokio::task::block_in_place(|| {
                handle.block_on(async move {
                    CdkWalletRepository::new_with_proxy(localstore, seed, proxy_url)
                        .await
                })
            }),
            Err(_) => {
    ...
    pub async fn add_mint(
        ...
        let config = if let Some(count) = target_proof_count {
            Some(cdk::wallet::multi_mint_wallet::WalletConfig::new()
