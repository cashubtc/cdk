...
use crate::types::bip321::ParsedPaymentInstruction;
...
///
/// ```text
/// val parsed = parseBip321PaymentInstruction("bitcoin:?creq=CREQB1...&lightning=lnbc100n1p...")
...
pub async fn parse_bip321_payment_instruction(
    ...
    let parsed = cdk::wallet::bip321::parse_payment_instruction(&instruction).await?;
    Ok(parsed.into())
}
...
pub async fn resolve_bip353_payment_instruction(
    ...
    let parsed = cdk::wallet::resolve_bip353_payment_instruction(&client, &address).await?;
    Ok(parsed.into())
}
