//! Binding-safe wallet object construction and configuration.

use std::sync::Arc;

use bip39::Mnemonic;
use cdk::wallet::advanced::{
    RateLimitConfig, WalletOpenAdvancedOptions as CdkWalletOpenAdvancedOptions,
};
use cdk::wallet::{
    Wallet as CdkWallet, WalletIdentity as CdkWalletIdentity,
    WalletOpenRequest as CdkWalletOpenRequest,
};

use crate::error::FfiError;
use crate::types::CurrencyUnit;

/// Application wallet for one mint and currency unit.
///
/// Open it with [`Wallet::open`] and use the request, session, and plan objects
/// from [`crate::wallet_api`]. All workflow behavior is implemented by the
/// wrapped [`cdk::Wallet`].
...
impl Wallet {
    /// Wrap an existing core wallet for FFI object-lifetime bridging.
    pub(crate) fn from_inner(inner: Arc<CdkWallet>) -> Self {
        Self { inner }
    }

    /// Access the core wallet behind this FFI object.
    pub(crate) fn inner(&self) -> &Arc<CdkWallet> {
        &self.inner
    }

    /// Construct a core wallet without contacting its mint.
    pub(crate) fn from_configuration(
        ...
        let identity = CdkWalletIdentity::new(
            mint_url.parse().map_err(|error: cdk::mint_url::Error| {
                FfiError::invalid_input(format!("Invalid URL: {error}"))
            })?,
            unit.into(),
        );
        let mut request = CdkWalletOpenRequest::new(identity, localstore, seed);
        if let Some(advanced) = config.advanced {
            let mut options = CdkWalletOpenAdvancedOptions::default();
            if let Some(target_proof_count) = advanced.target_proof_count {
                options = options.with_target_proof_count(target_proof_count as usize);
            }
            request = request.with_advanced(options);
        }
        let wallet = CdkWallet::open(request).map_err(FfiError::from)?;
        if let Some(rate_limit) = pace_with {
            wallet.set_rate_limiting_config(rate_limit);
        }
        if start_disabled {
...
/// Explicitly advanced proof-management options for standalone wallets.
...
pub struct WalletConfig {
    /// Explicit proof-management controls. Omit for strong defaults.
    #[uniffi(default = None)]
    pub advanced: Option<WalletOpenAdvancedOptions>,
