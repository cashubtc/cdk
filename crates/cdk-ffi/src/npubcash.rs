...
fn parse_nostr_secret_key(key: &str) -> Result<cdk_nostr::nostr_sdk::prelude::Keys, FfiError> {
    // Try parsing as nsec (bech32) first
    if key.starts_with("nsec") {
        cdk_nostr::nostr_sdk::prelude::Keys::parse(key)
            .map_err(|e| FfiError::internal(format!("Invalid nsec key: {}", e)))
    } else {
        // Try parsing as hex
        let secret_key = cdk_nostr::nostr_sdk::prelude::SecretKey::parse(key)
            .map_err(|e| FfiError::internal(format!("Invalid hex secret key: {}", e)))?;
        Ok(cdk_nostr::nostr_sdk::prelude::Keys::new(secret_key))
    }
}
