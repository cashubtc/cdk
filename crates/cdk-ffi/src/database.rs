...
impl WalletPostgresDatabase {
    ...
    pub fn new(url: String) -> Result<Arc<Self>, FfiError> {
        ...
        .map_err(|e| FfiError::Database { msg: e.to_string() })?;
        Ok(Arc::new(crate::WalletPostgresDatabase {
            inner: Arc::new(inner),
        }))
    }
    ...
    fn clone_as_trait(&self) -> Arc<dyn WalletDatabase> {
        ...
        let obj: Arc<dyn WalletDatabase> = Arc::new(crate::WalletPostgresDatabase {
...
pub fn create_wallet_db(backend: WalletDbBackend) -> Result<Arc<dyn WalletDatabase>, FfiError> {
    match backend {
        ...
        WalletDbBackend::Postgres { url } => {
            // Build a PG-backed database and wrap it with an adapter that implements the FFI trait
            // without changing the existing WalletDatabase trait surface.

            let inner = match tokio::runtime::Handle::try_current() {
                Ok(handle) => tokio::task::block_in_place(|| {
                    handle.block_on(async move {
                        cdk_postgres::new_wallet_pg_database(url.as_str()).await
                    })
                }),
                // Important: use a process-long runtime so background connection tasks stay alive.
                Err(_) => pg_runtime().block_on(async move {
                    cdk_postgres::new_wallet_pg_database(url.as_str()).await
                }),
            }
            .map_err(|e| FfiError::Database { msg: e.to_string() })?;

            let pg = Arc::new(WalletPostgresDatabase {
                inner: Arc::new(inner),
            });
