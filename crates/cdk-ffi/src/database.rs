    ...
    RM: DatabasePool + 'static,
{
    ...
    async fn add_saga(&self, saga_json: String) -> Result<(), FfiError> {
        let saga: WalletSaga =
            serde_json::from_str(&saga_json).map_err(|e| FfiError::internal(e))?;
        self.inner
            .add_saga(saga)
            .await
    ...
    async fn get_saga(&self, id: String) -> Result<Option<String>, FfiError> {
        let id = uuid::Uuid::parse_str(&id).map_err(|e| FfiError::internal(e))?;
        let result = self
            .inner
            .get_saga(&id)
            .await
        ...
        match result {
            Some(saga) => {
                let json = serde_json::to_string(&saga).map_err(|e| FfiError::internal(e))?;
                Ok(Some(json))
            }
            None => Ok(None),
        }
    ...
    async fn get_incomplete_sagas(&self) -> Result<Vec<String>, FfiError> {
        ...
        result
            .into_iter()
            .map(|saga| serde_json::to_string(&saga).map_err(|e| FfiError::internal(e)))
            .collect()
    }
    ...
    async fn reserve_proofs(
        ...
        let operation_id =
            uuid::Uuid::parse_str(&operation_id).map_err(|e| FfiError::internal(e))?;
        let cdk_ys: Result<Vec<cdk::nuts::PublicKey>, FfiError> =
            ys.into_iter().map(|pk| pk.try_into()).collect();
        let cdk_ys = cdk_ys?;
        self.inner
