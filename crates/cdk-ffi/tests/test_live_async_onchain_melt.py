#!/usr/bin/env python3
"""Live wallet-workflow test for an asynchronously finalized on-chain payment."""
...
async def wait_for_paid_mint_session(session):
    ...
    while time.monotonic() < deadline:
        ...
        if last_state.state == cdk_ffi.MintState.PAID:
            return last_state
...
async def test_live_async_onchain_payment():
    with tempfile.TemporaryDirectory(prefix="cdk-ffi-live-") as tmpdir:
        wallet = cdk_ffi.Wallet.open(
            cdk_ffi.WalletOpenRequest(
                ...
                config=cdk_ffi.WalletConfig(
                    advanced=cdk_ffi.WalletOpenAdvancedOptions(
        ...
        mint_session = await wallet.request_mint(
            cdk_ffi.MintRequest(
        ...
        assert claimed.amount.value > 0

        balance = await wallet.balance()
        assert balance.available.value >= MINT_AMOUNT_SAT

        quote_result = await wallet.quote_payment(
        ...
        if quote_result.is_OPTIONS():
            sessions = quote_result.sessions
        elif quote_result.is_SINGLE():
            sessions = [quote_result.session]
        else:
            raise AssertionError(f"unexpected quote result: {quote_result}")
        assert sessions
        ...
        confirmation = await plan.submit()
        ...
        if confirmation.is_PENDING():
            ...
            resumed = await wallet.resume_pending_payment(operation_id)
