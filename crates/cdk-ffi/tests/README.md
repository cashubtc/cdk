...
and callback behavior, wallet construction, manager construction, and the
absence of removed wallet objects and methods.
...
- `test_rate_limit.py` for offline workflow construction and API shape.
...
`cargo build --release -p cdk-ffi`. Use
`cargo check -p cdk-ffi --all-targets` to diagnose a Rust/UniFFI surface
change before regenerating bindings.
