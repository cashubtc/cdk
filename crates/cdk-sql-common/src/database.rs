...
#[derive(Debug)]
pub struct ConnnectionWithTransaction<DB, W>
where
...
impl<DB, W> ConnnectionWithTransaction<DB, W>
where
...
impl<DB, W> Drop for ConnnectionWithTransaction<DB, W>
where
    DB: DatabaseConnector,
    W: Debug + Deref<Target = DB> + DerefMut<Target = DB> + Send + Sync + 'static,
{
...
#[async_trait::async_trait]
impl<DB, W> DatabaseExecutor for ConnnectionWithTransaction<DB, W>
where
    DB: DatabaseConnector,
    W: Debug + Deref<Target = DB> + DerefMut<Target = DB> + Send + Sync + 'static,
{
