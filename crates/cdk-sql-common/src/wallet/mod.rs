...
;
 ...
 ;
    ...
    /// TODO: Improve this to offload the matches_condition to the SQL engine, also add limit property
    ...
    RM: DatabasePool + 'static,
{
    ...
    async fn begin_db_transaction<'a>(
        &'a self,
    ) -> Result<Box<dyn WalletDatabaseTransaction<'a, Error> + Send + Sync + 'a>, Error> {
        let tx = SQLWalletTransaction {
