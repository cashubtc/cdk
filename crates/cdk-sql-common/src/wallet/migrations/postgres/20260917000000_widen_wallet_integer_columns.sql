-- Widen the columns whose value can outgrow a signed 32-bit one: u64 amounts,
-- unix timestamps, and the fields the mint controls. mint_time, input_fee_ppk
-- and final_expiry are u64 straight off the wire, and estimated_blocks and
-- fee_index are u32 the wallet stores without a range check, so a peer rather
-- than a clock decides how large they get.
--
