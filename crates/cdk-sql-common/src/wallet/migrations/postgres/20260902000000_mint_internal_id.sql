...
-- PostgreSQL can add the identity column in place, but the primary key has to
-- move off mint_url, and keyset's foreign key points at that key, so it goes
-- first.
...
-- melt_quote.mint_url was nullable, so its mint_id takes no NOT NULL: the rows
-- that had no URL still have no mint.
