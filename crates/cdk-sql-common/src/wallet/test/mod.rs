...
/// The migration under test, by the name `build.rs` records it under.
const MINT_INTERNAL_ID_NAME: &str = "20260902000000_mint_internal_id.sql";

/// The leading integer of a migration name, which is the order `build.rs` intends.
///
/// `MIGRATIONS` itself is sorted by path, so the dialect directory sorts ahead of the filename and
/// the two copies of one migration are not adjacent. Comparing prefixes rather than positions is
/// what makes the cut below dialect-independent.
fn order_key(name: &str) -> u64 {
    name.split('_')
        .next()
...
{
    let cutoff = order_key(MINT_INTERNAL_ID_NAME);
    let earlier = MIGRATIONS
        .iter()
        .filter(|(_, name, _)| order_key(name) < cutoff)
        .copied()
        .collect::<Vec<_>>();

    migrate(conn, dialect, &earlier).await?;
