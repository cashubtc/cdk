    ...
    RM: DatabasePool + 'static,
{
    ...
    async fn add_completed_operation(
        ...
        .bind_u64("completed_at", operation.completed_at().unwrap_or(unix_time()))?
        .bind_u64("total_issued", operation.total_issued().to_u64())?
        .bind_u64("total_redeemed", operation.total_redeemed().to_u64())?
        .bind_u64("fee_collected", operation.fee_collected().to_u64())?
        .bind_u64("payment_amount", operation.payment_amount().map(|a| a.to_u64()))?
        .bind_u64("payment_fee", operation.payment_fee().map(|a| a.to_u64()))?
