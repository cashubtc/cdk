...
fn main() {
    ...
    for migration_path in find_migrations_dirs(root) {
        ...
        // Write a manifest file to the source tree for version control tracking.
        // This makes migration ordering visible in diffs and code review.
        {
            let manifest_path = migration_path.join("manifest.txt");
            let mut manifest_file =
                File::create(&manifest_path).expect("Failed to create manifest.txt");
            writeln!(
                manifest_file,
            ...
            writeln!(
                manifest_file,
                "# Rebuild with: cargo build -p cdk-sql-common"
            )
            .unwrap();
            for path in &files {
                ...
                writeln!(manifest_file, "{prefix} {rel_name}").unwrap();
            }
        }
