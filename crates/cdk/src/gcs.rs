...
impl GCSFilter {
    ...
    pub fn create(
        ...
        if (items.len() as u64).checked_ilog2().unwrap_or(0) > 31 {
            return Err(GCSError::new("number of elements must be smaller than 2^32"));
        }
...
mod tests {
    ...
    fn test_gcs_filter() {
        ...
        println!("num_items = {}, item_size = {}, filter_size = {}", 
                 num_items, item_size, gcs_filter.len());
        ...
        assert!(!results[&non_existent_item], "Non-existent item was incorrectly found in the GCS filter");
    }
}