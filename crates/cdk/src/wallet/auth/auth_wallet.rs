...
impl AuthWallet {
    ...
    pub async fn refresh_access_token(&self, client_id: String) -> Result<(), Error> {
        if let Some(oidc) = self.oidc_client.read().await.as_ref() {
            if let Some(refresh_token) = self.get_refresh_token().await {
                let token_response = oidc.refresh_access_token(client_id, refresh_token).await?;
    ...
    pub async fn is_protected(&self, method: &ProtectedEndpoint) -> Option<AuthRequired> {
        ...
        println!("protected: {:?}", protected_endpoints);
