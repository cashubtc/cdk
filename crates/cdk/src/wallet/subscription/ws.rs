...
use cdk_common::pub_sub::remote_consumer::{MessageToTransportLoop, SubscribeMessage};
use cdk_common::pub_sub::{Error as PubsubError, Pubsub};
#[cfg(feature = "auth")]
...
pub(crate) async fn long_connection(
    ...
    reply_to: Arc<Pubsub<MintSubTopics>>,
) -> Result<(), PubsubError> {
    let mut url = client
        .mint_url
        .join_paths(&["v1", "ws"])
