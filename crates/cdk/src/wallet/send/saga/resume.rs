    ...
    pub(crate) async fn resume_send_saga(
        ...
        match state {
            ...
            SendSagaState::ProofsReserved => {
                // Before durable plans were introduced, this state stored
                // Send data. Existing wallets must still release those funds.
                if !matches!(
                    saga.data,
                    OperationData::PreparedSend(_) | OperationData::Send(_)
                ) {
                    return Err(Error::Custom(format!(
                        "Invalid prepared operation data for send saga {}",
                        saga.id
    ...
    async fn claim_and_compensate_send(
        ...
        if saga.mint_url != self.mint_url
            ...
            || !(matches!(saga.data, OperationData::PreparedSend(_))
                || (current_state == SendSagaState::ProofsReserved
                    && matches!(saga.data, OperationData::Send(_))))
        {
            return Ok(false);
        }
...
mod tests {
    ...
    #[tokio::test]
    async fn recover_reserved_send_written_by_main() {
        ...
        .expect("decode main-branch send record");
