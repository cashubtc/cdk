...
impl WalletRepository {
    ...
    pub async fn receive_nostr_request(
        ...
        let wallet = self
            ...
            .await?;
        // Do not replace the input ownership of an unresolved receive.
        if wallet
            .localstore
    ...
    // Classify rejection before deciding whether to keep the relay listener alive.
    async fn receive_nostr_request_event(
        &self,
        id: &str,
        info: &NostrWaitInfo,
        payload: PaymentRequestPayload,
        ...
        if self
            .preflight_nostr_stream_payload(&info.validation(), &payload)
            .await?
            .is_none()
        {
            return Ok(None);
    ...
    pub async fn check_nostr_request(&self, id: &str) -> Result<NostrRequest, Error> {
        ...
        for event in events {
            ...
            if self
                .receive_nostr_request_event(id, &info, payload)
                .await?
                .is_some()
            {
                break;
    ...
    pub async fn wait_for_nostr_request(&self, id: &str) -> Result<Amount, Error> {
        ...
        loop {
            tokio::select! {
                ...
                item = stream.next() => {
                    match item {
                        Some(Ok(payload)) => match self.receive_nostr_request_event(id, &info, payload).await? {
                            Some(amount) => { cancel.cancel(); return Ok(amount); }
                            None => {},
                        },
...
mod tests {
    ...
    async fn offline_payment_is_fetched_and_received_after_restart() {
        ...
        for wait in [false, true] {
            ...
            connector.set_check_state_response(Ok(crate::nuts::CheckStateResponse {
                states: vec![crate::nuts::ProofState {
                    y: payment.proofs[0].y().unwrap(),
                    state: crate::nuts::State::Unspent,
                    witness: None,
                }],
            }));
            let result = if wait {
    ...
    #[tokio::test]
    async fn failed_output_restore_keeps_request_receiving_until_retry_succeeds() {
        ...
        let connector = add_mock_wallet(&repo).await;
        let payment = payload(&info);
        let y = payment.proofs[0].y().unwrap();
        connector.set_post_swap_response(Err(Error::HttpError(None, "disconnected".to_owned())));
        assert!(repo.receive_nostr_request(id, payment).await.is_err());
        connector.set_post_swap_response(Err(Error::HttpError(None, "disconnected".to_owned())));
        ...
        connector._set_restore_response(Err(Error::HttpError(Some(404), "unavailable".to_owned())));
        let result = repo.check_nostr_request(id).await.unwrap();
        let operation_id = result.receive_operation_id.unwrap();
        assert_eq!(result.status, NostrRequestStatus::Receiving);
        ...
        let wallet = repo
            .get_wallet(&test_mint_url(), &CurrencyUnit::Sat)
            .await
            .unwrap();
        assert_eq!(wallet.total_balance().await.unwrap(), Amount::ZERO);
        // A later restore succeeds using the original output messages.
        connector.set_post_swap_response(Err(Error::HttpError(None, "disconnected".to_owned())));
        connector.set_check_state_response(Ok(CheckStateResponse {
            states: vec![ProofState {
                y,
        ...
        connector._set_restore_response(Ok(crate::nuts::RestoreResponse {
            outputs: connector.captured_swap_requests()[0].outputs().clone(),
            signatures: swap_response().signatures,
        }));
        let result = repo.check_nostr_request(id).await.unwrap();
        assert_eq!(result.status, NostrRequestStatus::Completed);
        assert_eq!(result.receive_operation_id, Some(operation_id));
    ...
    #[tokio::test]
    async fn same_proofs_cannot_replace_an_unresolved_request() {
        let db = create_test_db().await;
        let repo = repository(db.clone()).await;
        let (_, first) = repo.create_request(params()).await.unwrap();
        let (_, second) = repo.create_request(params()).await.unwrap();
        let first = first.unwrap();
        let second = second.unwrap();
        let first_id = first.request.payment_id.as_deref().unwrap();
        let second_id = second.request.payment_id.as_deref().unwrap();
        let connector = add_mock_wallet(&repo).await;
        let mut payment = payload(&first);
        connector.set_post_swap_response(Err(Error::HttpError(None, "offline".to_owned())));
        assert!(repo
            .receive_nostr_request(first_id, payment.clone())
            .await
            .is_err());
        payment.id = Some(second_id.to_owned());
        assert!(matches!(
            repo.receive_nostr_request(second_id, payment).await,
            Err(Error::PaymentPending)
        ));
        assert_eq!(connector.captured_swap_requests().len(), 1);
        let original = repo.get_nostr_request(first_id).await.unwrap().unwrap();
        let operation_id = original.receive_operation_id.unwrap();
        assert_eq!(original.status, NostrRequestStatus::Receiving);
        assert_eq!(
            repo.get_nostr_request(second_id)
                .await
                .unwrap()
                .unwrap()
                .status,
        ...
        assert_eq!(
            db.get_reserved_proofs(&operation_id).await.unwrap().len(),
            1
        );
        connector.set_post_swap_response(Ok(swap_response()));
        let result = repo.check_nostr_request(first_id).await.unwrap();
        assert_eq!(result.status, NostrRequestStatus::Completed);
        assert_eq!(result.receive_operation_id, Some(operation_id));
        assert_eq!(result.received, Some(Amount::from(1)));
        let wallet = repo
    ...
    #[tokio::test]
    async fn persisted_listener_skips_invalid_proofs_but_returns_uncertain_errors() {
        use crate::nuts::{CheckStateResponse, ProofState, State};
        ...
        for uncertain in [false, true] {
            ...
            connector.set_check_state_response(states(State::Spent));
            assert_eq!(
                repo.receive_nostr_request_event(id, &info, payment.clone())
                    .await
                    .unwrap(),
                None
            );
            assert!(connector.captured_swap_requests().is_empty());
            connector.set_check_state_response(states(State::Unspent));
            connector.set_post_swap_response(Err(if uncertain {
                Error::Timeout
            } else {
                Error::DHKE(crate::dhke::Error::TokenNotVerified)
            }));
            let result = repo
                .receive_nostr_request_event(id, &info, payment.clone())
                .await;
            if uncertain {
                assert!(matches!(result, Err(Error::Timeout)));
                assert_eq!(
                    repo.get_nostr_request(id).await.unwrap().unwrap().status,
                    NostrRequestStatus::Receiving
                );
            } else {
                ...
                connector.set_post_swap_response(Ok(swap_response()));
                assert_eq!(
                    repo.receive_nostr_request_event(id, &info, payment)
                        .await
                        .unwrap(),
                    Some(Amount::from(1))
                );
