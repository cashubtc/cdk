...
//! payment string into [`ParsedPaymentInstruction`].
//!
//! # Creating
//!
...
//!
//! // Parse it back
//! let parsed = parse_payment_instruction(&uri).await?;
...
impl Bip321UriBuilder {
    ...
    pub fn with_cashu_request(mut self, request: &PaymentRequest) -> Result<Self, Error> {
        let bech32_str = request
            .to_bech32_string()
            .map_err(|e| Error::Custom(format!("Failed to encode payment request: {e}")))?;
...
pub async fn parse_payment_instruction(
    ...
    let parsed = PaymentInstructions::parse(instruction, network, &resolver, false)
        .await
        .map_err(|e| Error::Custom(format!("Failed to parse payment instruction: {e:?}")))?;

    convert_payment_instructions(parsed)
}

/// Parse a concrete BIP 321 `bitcoin:` URI that has already been resolved.
///
/// This is useful for flows like BIP-353 where a human-readable address is first resolved to
/// a concrete `bitcoin:` URI and then parsed into the shared [`ParsedPaymentInstruction`] model.
pub async fn parse_resolved_payment_instruction(
    instruction: &str,
) -> Result<ParsedPaymentInstruction, Error> {
    parse_payment_instruction(instruction).await
}
...
pub async fn resolve_bip353_payment_instruction(
    ...
    parse_resolved_payment_instruction(&resolved_uri)
        .await
        .map_err(|e| {
            tracing::error!(
            ...
            Error::Bip353Resolve(format!("Invalid resolved bitcoin URI: {e}"))
        })
}
    ...
    match parsed {
        PaymentInstructions::FixedAmount(fixed) => {
            ...
            extract_payment_methods(
                fixed.methods(),
                fixed.recipient_description().map(String::from),
                max_amount_msats,
                false,
            )
        }
        PaymentInstructions::ConfigurableAmount(configurable) => extract_payment_methods(
            ...
            configurable.recipient_description().map(String::from),
            None,
            true,
        ),
    }
    ...
    for method in methods {
        match method {
            BpiPaymentMethod::Cashu(cashu_req) => {
                ...
                let cdk_req = PaymentRequest::from_str(&bech32_str).map_err(|e| {
                    Error::Custom(format!("Failed to convert cashu payment request: {e}"))
                })?;
...
mod tests {
    ...
    async fn test_parse_standalone_cashu_request() {
        let parsed = parse_payment_instruction(TEST_CREQ)
            .await
    ...
    async fn test_parse_bip321_uri_with_cashu() {
        ...
        let parsed = parse_payment_instruction(&uri)
            .await
    ...
    async fn test_parse_standalone_bolt11() {
        let result = parse_payment_instruction(TEST_BOLT11).await;
