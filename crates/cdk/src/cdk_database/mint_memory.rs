...
use cdk_common::{AuthProof, AuthRequired, MintInfo, ProtectedEndpoint};
...
impl MintAuthDatabase for MintMemoryAuthDatabase {
    ...
    async fn add_protected_endpoints(
        &self,
        _protected_endpoints: HashMap<ProtectedEndpoint, AuthRequired>,
    ) -> Result<(), Self::Err> {
        todo!()
    }
    async fn remove_protected_endpoints(
        &self,
        _protected_endpoints: Vec<ProtectedEndpoint>,
    ) -> Result<(), Self::Err> {
        todo!()
    }
    async fn get_auth_for_endpoint(
        &self,
        _protected_endpoint: ProtectedEndpoint,
