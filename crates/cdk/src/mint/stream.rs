...
use cdk_common::ws::{
    ...
    WsMethodRequest, WsRequest, WsResponseResult, WsSubscribeResponse, WsUnsubscribeResponse,
};
use tokio::sync::mpsc;
            ...
            subscriptions.insert(
                ...
                tokio::spawn(async move {
                    while let Some(event) = subscription.recv().await {
                        // Best-effort: the publisher channel is bounded (100) and
                        // shared by every subscription on this connection, so a
                        // burst drops notifications rather than blocking. The
                        // wallet's HTTP poll fallback is the safety net.
                        let _ = publisher.try_send((sub_id_for_sender.clone(), event.into_inner()));
                    }
                }),
            );
