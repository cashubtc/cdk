...
use cdk_common::melt::{MeltQuoteRequest, MeltRequest};
...
use cdk_common::{
    ...
    MeltQuoteBolt12Request, PaymentMethod,
};
...
use crate::cdk_payment::{
    Bolt11OutgoingPaymentOptions, Bolt12OutgoingPaymentOptions, MakePaymentResponse, MintPayment,
    OutgoingPaymentOptions, PaymentQuoteOptions,
};
use crate::mint::nut11::{enforce_sig_flag, EnforceSigFlag};
use crate::mint::verification::Verification;
use crate::nuts::{MeltQuoteState, PublicKey, SigFlag, State};
...
impl Mint {
    ...
    #[instrument(skip_all)]
    pub async fn process_unpaid_melt(&self, melt_request: &MeltRequest) -> Result<(), Error> {
        let input_ys = melt_request.inputs().ys()?;

        self.localstore
            .remove_proofs(&input_ys, Some(*melt_request.quote_id()))
            .await?;

        self.localstore
            .update_melt_quote_state(melt_request.quote_id(), MeltQuoteState::Unpaid)
            .await?;

        if let Ok(Some(quote)) = self
            .localstore
            .get_melt_quote(melt_request.quote_id())
            .await
        {
            self.pubsub_manager
    ...
    pub async fn process_melt_request(
        ...
        tracing::debug!("Processing melt quote: {}", melt_request.quote_id());
        ...
        self.localstore
            .update_melt_quote_state(melt_request.quote_id(), MeltQuoteState::Paid)
            .await?;
    ...
    pub async fn verify_melt_request(
        ...
        let quote_id = melt_request.quote_id();
        let state = self
            .localstore
        ...
        if let Some(outputs) = &melt_request.outputs() {
            if !outputs.is_empty() {
                let Verification {
                    amount: _,
                    unit: output_unit,
                } = self.verify_outputs(outputs.as_slice()).await?;
        ...
        tracing::debug!(
            "Verified melt quote: {}",
            melt_request.quote_id().to_string()
        );
        Ok(quote)
    }
    /// Melt
    ...
    pub async fn melt(
        ...
        async fn check_payment_state(
                ...
                .await
            {
                ...
                Err(check_err) => {
                    ...
                    anyhow::bail!("Could not check payment status")
                }
            }
        }
        ...
        let (preimage, amount_spent_quote_unit) = match settled_internally_amount {
            ...
            None => {
                ...
                let pre = match ln
                    .make_payment(&quote.clone().unit, quote.clone().try_into()?)
                    .await
                {
                    Ok(pay)
