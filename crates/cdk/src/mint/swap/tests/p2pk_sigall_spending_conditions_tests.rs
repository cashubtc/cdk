...
async fn test_p2pk_sig_all_mixed_proofs_different_data() {
    ...
    let (new_outputs, _) = create_test_blinded_messages(mint.clone(), total_amount)
        .await
        .unwrap();
    ...
    let result = mint.clone().process_swap_request(swap_request_mixed).await;
    ...
    let (alice_new_outputs, _) = create_test_blinded_messages(mint.clone(), alice_input_amount)
        .await
        .unwrap();
    ...
    let (bob_new_outputs, _) = create_test_blinded_messages(mint.clone(), bob_input_amount)
        .await
        .unwrap();
...
async fn test_sig_all_should_reject_if_the_output_amounts_are_swapped() {
    ...
    let result = mint.clone().process_swap_request(swap_request).await;
    assert!(
        result.is_ok(),
