...
use cdk_common::{
    CurrencyUnit, MintQuoteBolt11Request, MintQuoteBolt11Response, MintQuoteBolt12Response,
    MintQuoteState, MintRequest, MintResponse, NotificationPayload, PaymentMethod, PublicKey,
};
...
impl Mint {
    ...
    /// Updates an existing mint quote in the database
    ///
    /// # Arguments
    /// * `quote` - The mint quote with updated information
    ///
    /// # Returns
    ...
    /// Retrieves all pending mint quotes from the database
    ///
    /// Pending quotes are those that have been created but are still being processed.
    ///
    /// # Returns
    /// * `Vec<MintQuote>` - List of pending mint quotes
    /// * `Error` if database access fails
    #[instrument(skip_all)]
    ...
    /// Retrieves all unpaid mint quotes from the database
    ///
    /// Unpaid quotes are those that have been created but haven't received payment yet.
    ///
    /// # Returns
    /// * `Vec<MintQuote>` - List of unpaid mint quotes
    /// * `Error` if database access fails
    #[instrument(skip_all)]
    ...
    pub async fn check_mint_quote(&self, quote_id: &Uuid) -> Result<MintQuoteResponse, Error> {
        ...
        let quote = if quote.state() != state {
            self.localstore
                .get_mint_quote(quote_id)
                .await?
                .ok_or(Error::UnknownQuote)?
