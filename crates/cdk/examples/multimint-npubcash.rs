//! Example: WalletManager with NpubCash - Switching Active Mints
//!
//! This example demonstrates:
//! 1. Creating a WalletManager with multiple mints
//! 2. Using NpubCash integration with the WalletManager API
//! 3. Switching the active mint for NpubCash deposits
//! 4. Receiving payments to different mints and verifying balances
//!
//! Key concept: Since all wallets in a WalletManager share the same seed, they all
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== WalletManager with NpubCash Example ===\n");

    // -------------------------------------------------------------------------
    // Step 1: Create WalletManager and add mints
    // -------------------------------------------------------------------------
    println!("Step 1: Setting up WalletManager...\n");
    ...
    wallet_manager.register_mint(mint_url_1.clone()).await?;
    wallet_manager.register_mint(mint_url_2.clone()).await?;
    ...
    let wallet = wallet_manager
        .wallet(WalletIdentity::new(mint_url_1.clone(), CurrencyUnit::Sat))
        .await?;
    wallet
        .advanced()
        .enable_npubcash(NPUBCASH_URL.to_string())
        .await?;
    ...
    // The stream is for the wallet manager so it handles switching mints automatically
    ...
    let total = wallet.balance().await?.available;
