...
use crate::web::templates::{
    error_message, form_card, format_msats_as_btc, format_sats_as_btc, info_card, layout_with_status, is_node_running,
...
pub async fn payments_page(
    ...
    Ok(Html(layout_with_status("Payment History", content, is_running).into_string()))
}
...
pub async fn send_payments_page(
    ...
    let is_running = is_node_running(&state.node.inner);
    Ok(Html(layout_with_status("Send Payments", content, is_running).into_string()))
}
...
pub async fn post_pay_bolt11(
    ...
    let invoice = match Bolt11Invoice::from_str(form.invoice.trim()) {
        ...
        Err(e) => {
            ...
            return Ok(Response::builder()
                ...
                .body(Body::from(layout_with_status("Payment Error", content, true).into_string()))
                .unwrap());
        }
    };
    ...
    Ok(Response::builder()
        .header("content-type", "text/html")
        .body(Body::from(layout_with_status("Payment Result", content, true).into_string()))
        .unwrap())
}
