    ...
    async fn handle_payment_received(
        ...
        let payment_identifier = match payment_details.kind {
            PaymentKind::Bolt11 { hash, .. } => {
                ...
                match database.get_quote_id_by_payment_hash(&hash).await {
                    Ok(Some(quote_id)) => {
                        tracing::info!(
            ...
            PaymentKind::Bolt12Offer { offer_id, .. } => {
                ...
                match database.get_quote_id_by_offer_id(&offer_id).await {
                    Ok(Some(quote_id)) => {
                        tracing::info!(
    ...
    async fn create_incoming_payment_request(
        ...
        match options {
            IncomingPaymentOptions::Bolt11(bolt11_options) => {
                ...
                self.database
                    .store_bolt11_request(&payment_hash, quote_id)
                    .await
            ...
            IncomingPaymentOptions::Bolt12(bolt12_options) => {
                ...
                self.database
                    .store_bolt12_request(&offer.id(), quote_id)
                    .await
                    .map_err(|e| payment::Error::Custom(e.to_string()))?;
    ...
    async fn make_payment(
        ...
        match options {
            OutgoingPaymentOptions::Bolt11(bolt11_options) => {
                ...
                if let Ok(Some(payment_status)) = self
                    .database
                ...
                self.database
                    .store_quote_payment(quote_id, initial_payment_status)
                    .await
                    .map_err(|e| payment::Error::Custom(e.to_string()))?;
                ...
                self.database
                    .store_quote_payment(quote_id, final_payment_status)
                    .await
                    .map_err(|e| payment::Error::Custom(e.to_string()))?;
