...
use cdk::cdk_payment::{
    ...
    OutgoingPaymentOptions, PaymentQuoteResponse, WaitPaymentResponse,
};
use cdk::nuts::{CurrencyUnit, MeltOptions, MeltQuoteState};
use cdk::types::FeeReserve;
use cdk::util::{hex, unix_time};
use cdk::{ensure_cdk, Bolt11Invoice};
                ...
                loop {
                    tokio::select! {
                        ...
                        result = cln_client.call(cln_rpc::Request::WaitAnyInvoice(WaitanyinvoiceRequest {
                            ...
                            tracing::warn!("Received payment of {:?} msats for {}", amount_msats, payment_hash);
                        ...
                        MeltOptions::Amountless { amountless } => {
                                ...
                                bolt11_options.bolt11.amount_milli_satoshis()
                            {
                                ensure_cdk!(
                                    ...
                                    cdk_payment::Error::AmountMismatch
                                );
                            }
                            amount_msat
                        }
        ...
        let invoice = match options {
            ...
            OutgoingPaymentOptions::Bolt12(bolt12_options) => {
                ...
                let decode_response = self
                    .decode_string(String::from_utf8(bolt12_invoice.clone())?)
                    .await?;
                ...
                String::from_utf8(bolt12_invoice)?
            }
        };
    ...
    async fn check_outgoing_unpaided(
        ...
        match pay_state.status {
            ...
            MeltQuoteState::Paid => {
                ...
                Err(cdk_payment::Error::InvoiceAlreadyPaid)
            }
            MeltQuoteState::Pending => {
                tracing::debug!("Melt attempted on invoice already pending");
                Err(cdk_payment::Error::InvoicePaymentPending)
            }
        }
    }
