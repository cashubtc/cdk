...
type CacheEvent<T> = HashMap<<<T as Topic>::Event as Event>::Topic, <T as Topic>::Event>;
    ...
    T: Transport + 'static,
{
    ...
    stream_ctrl: RwLock<Option<mpsc::Sender<StreamCtrl<T::Topic>>>>,
    ...
    cached_events: Arc<RwLock<CacheEvent<T::Topic>>>,
}
    ...
    T: Transport + 'static,
{
    ...
    previous_messages: VecDeque<<T::Topic as Topic>::Event>,
    ...
    T: Transport + 'static,
{
    ...
    pub async fn recv(&mut self) -> Option<<T::Topic as Topic>::Event> {
    ...
    pub fn try_recv(&mut self) -> Option<<T::Topic as Topic>::Event> {
        if let Some(event) = self.previous_messages.pop_front() {
            Some(event)
        } else {
            self.inner.try_recv()
    ...
    pub fn name(&self) -> &<T::Topic as Topic>::SubscriptionName {
        self.inner.name()
    }
}
...
{
    inner: Arc<Pubsub<T>>,
    cached_events: Arc<RwLock<CacheEvent<T>>>,
}

impl<T> InternalRelay<T>
where
    T: Topic + 'static,
{
    ...
    T: Transport + 'static,
{
    ...
    pub fn new(transport: T, prefer_polling: bool, topic: T::Topic) -> Arc<Self> {
    ...
    #[inline(always)]
    fn message_to_stream(&self, message: StreamCtrl<T::Topic>) -> Result<(), Error> {
...
pub trait Transport: Send + Sync {
    ...
    async fn stream(
        &self,
        subscribe_changes: mpsc::Receiver<StreamCtrl<Self::Topic>>,
...
mod tests {
    ...
    #[derive(Clone, Debug, Serialize, Eq, PartialEq, Ord, PartialOrd, Hash, Deserialize)]
    enum IndexTest {
        Foo(u64),
        Bar(u64),
    }
    ...
    impl Topic for CustomTopic {
        ...
        // For tests we don't need historical fetches; do nothing.
    ...
    struct TestTransport {
        ...
        observe_ctrl_tx: mpsc::Sender<StreamCtrl<CustomTopic>>,
    ...
    impl Transport for TestTransport {
        ...
        async fn stream(
            &self,
            mut subscribe_changes: mpsc::Receiver<StreamCtrl<Self::Topic>>,
            topics: Vec<SubscribeMessage<Self::Topic>>,
            reply_to: InternalRelay<Self::Topic>,
        ) -> Result<(), Error> {
    ...
    async fn expect_ctrl(
        rx: &mut mpsc::Receiver<StreamCtrl<CustomTopic>>,
        dur_ms: u64,
        pred: impl Fn(&StreamCtrl<CustomTopic>) -> bool,
    ) -> StreamCtrl<CustomTopic> {
