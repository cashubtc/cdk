//! ConditionsDatabase tests (NUT-28)
    ...
    DB: Database<Error> + ConditionsDatabase<Err = Error>,
{
    ...
    let all = db.get_conditions().await.unwrap();
    assert_eq!(all.len(), 2);
}
    ...
    DB: Database<Error> + ConditionsDatabase<Err = Error>,
{
    ...
    db.add_conditional_keyset_info(&cond.condition_id, "YES", &"e1".repeat(32), &ks_yes)
        .await
        .unwrap();
    db.add_conditional_keyset_info(&cond.condition_id, "NO", &"e2".repeat(32), &ks_no)
        .await
        .unwrap();
    ...
    DB: Database<Error> + ConditionsDatabase<Err = Error>,
{
    ...
    db.add_conditional_keyset_info(&cond.condition_id, "YES", &oc_id, &keyset_id)
        .await
        .unwrap();
