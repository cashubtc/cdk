//! Database module for LNbits backend operations.
//!
//! This module provides database storage and retrieval functionality for LNbits lightning backend,
//! including management of incoming and outgoing payments, payment status tracking, and
//! quote ID mappings for BOLT11 payments.
...
const PRIMARY_NAMESPACE: &str = "cdk_lnbits_lightning_backend";
...
/// Payment identifier for incoming payments (LNbits supports BOLT11 only).
///
/// Currently, LNbits backend only supports BOLT11 payments, so this enum
...
/// Payment status information for outgoing payments from LNbits.
///
...
/// Database operation errors specific to LNbits backend.
///
/// This error type encapsulates all possible database-related failures
/// that can occur during LNbits backend operations.
...
/// Database wrapper for LNbits backend operations.
///
/// Provides a high-level interface for storing and retrieving payment information
/// and quote mappings. Handles both incoming and outgoing payments with support
/// for BOLT11 protocol through the LNbits API.
///
/// # Example
/// ```rust,ignore
/// use cdk_lnbits::database::Database;
///
...
impl Database {
    ...
    /// the corresponding quote when a BOLT11 payment is received via LNbits.
    ///
    /// # Arguments
    ...
    ///
    /// Performs reverse lookup to find the payment identifier (BOLT11 hash)
    /// associated with a quote ID. Used to determine the original payment hash.
    ///
    /// # Arguments
    ...
    ///
    /// This stores the payment status information for outgoing payments via LNbits,
    /// allowing tracking of payment state, fees, and completion status.
    ///
    /// # Arguments
...
mod tests {
    ...
    fn test_payment_status_serialization() {
        ...
        let payment_status = PaymentStatus {
            ...
            payment_hash,
            payment_proof: Some("preimage_123".to_string()),
            total_spent: Amount::from(1000u64),
        };

        // Test serialization and deserialization
        let serialized = serde_json::to_vec(&payment_status).expect("Failed to serialize");
        let deserialized: PaymentStatus =
            serde_json::from_slice(&serialized).expect("Failed to deserialize");

        assert_eq!(deserialized.status, payment_status.status);
        assert_eq!(deserialized.payment_hash, payment_status.payment_hash);
        assert_eq!(deserialized.payment_proof, payment_status.payment_proof);
        assert_eq!(deserialized.total_spent, payment_status.total_spent);
    }

    #[test]
    fn test_incoming_payment_identifier_serialization() {
        let payment_hash = [2u8; 32];
        let identifier = IncomingPaymentIdentifier::Bolt11PaymentHash(payment_hash);

        // Test serialization and deserialization
        let serialized = serde_json::to_vec(&identifier).expect("Failed to serialize");
        let deserialized: IncomingPaymentIdentifier =
            serde_json::from_slice(&serialized).expect("Failed to deserialize");

        match deserialized {
            IncomingPaymentIdentifier::Bolt11PaymentHash(hash) => {
