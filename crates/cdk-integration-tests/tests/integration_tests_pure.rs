...
async fn test_swap_to_send() {
    ...
    let keysets_info = wallet_alice.get_mint_keysets(true).await.unwrap();
...
async fn test_p2pk_send_force_swap_with_fees() {
    ...
    let keysets_info = wallet.get_mint_keysets(true).await.unwrap();
    let token_proofs = token.proofs(&keysets_info).unwrap();
    assert_eq!(
        send_amount,
...
async fn test_p2pk_send_force_swap_with_fees_include_fee() {
    ...
    let keysets_info = wallet_sender.get_mint_keysets(true).await.unwrap();
    let token_proofs = token.proofs(&keysets_info).unwrap();
    assert_eq!(
