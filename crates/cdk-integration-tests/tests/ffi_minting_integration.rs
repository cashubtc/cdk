...
use cdk_ffi::{
    MintRequest, MintState, PaymentMethod, WalletConfig, WalletOpenAdvancedOptions,
    WalletOpenRequest,
};
...
async fn create_test_ffi_wallet() -> FfiWallet {
    ...
    let config = WalletConfig {
        advanced: Some(WalletOpenAdvancedOptions {
...
async fn test_ffi_full_minting_flow() {
    ...
    let mint_result = timeout(Duration::from_secs(30), async {
        ...
        loop {
            ...
            match current.state {
                MintState::Paid => break session.claim().await.expect("Failed to claim quote"),
                MintState::Unpaid => {
                    tokio::time::sleep(Duration::from_millis(2000)).await;
                }
                MintState::Issued => break session.claim().await.expect("Failed to load receipt"),
            }
        }
    })
    .await
    ...
    let total_minted = mint_result.amount.value;
    assert_eq!(
...
async fn test_ffi_mint_quote_creation() {
    ...
    for amount_value in test_amounts {
        ...
        assert_eq!(quote.state, MintState::Unpaid);
...
async fn test_ffi_minting_error_handling() {
    ...
    let invalid_mint_result = wallet
        .resume_mint("non-existent-quote-id".to_string())
        .await;
    assert!(
