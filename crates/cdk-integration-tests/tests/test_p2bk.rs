//! Integration tests for Pay-to-Blinded-Key (NUT-28)
//!
...
use cashu::nuts::nut28::{derive_signing_key_bip340, ecdh_kdf};
use cashu::{
...
async fn test_p2bk_payment_request() {
    ...
    // Enable NUT-28 (P2BK) and build the request
    let _payment_request_str = payment_request.nut28(true).build().to_string();
...
async fn test_p2bk_ecdh_key_derivation() {
    ...
    let blinded_pubkey =
        cashu::nuts::nut28::blind_public_key(&receiver_pubkey, &blinding_scalar_sender)
