...
async fn start_ldk_mint(
    ...
    let handle = tokio::spawn(async move {
        ...
        match cdk_mintd::run_mintd_with_shutdown(&ldk_work_dir, &settings, shutdown_future, None)
            .await
        {
...
async fn main() -> Result<()> {
    ...
    // Start LDK mint
    let ldk_handle = start_ldk_mint(&temp_dir, args.ldk_port, shutdown_clone.clone()).await?;
    ...
    match tokio::try_join!(
        shared::wait_for_mint_ready_with_shutdown(args.lnd_port, 100, Arc::clone(&cancel_token)),
        shared::wait_for_mint_ready_with_shutdown(args.ldk_port, 100, Arc::clone(&cancel_token)),
        shared::wait_for_mint_ready_with_shutdown(args.cln_port, 100, Arc::clone(&cancel_token)),
    ) {
