...
use ln_regtest_rs::{
    cln::Clnd,
    ln_client::{ClnClient, LightningClient},
};
...
async fn main() -> Result<()> {
    ...
    let sqlx_filter = "sqlx=warn,h2=warn,hyper=warn";
    ...
    match mint_db_kind.as_str() {
        "MEMORY" => {
            start_cln_mint(cln_one_dir, addr, port, MintMemoryDatabase::default()).await?;
        }
        "SQLITE" => {
            let sqlite_db = MintSqliteDatabase::new(&db_path).await?;
            sqlite_db.migrate().await;
            start_cln_mint(cln_one_dir, addr, port, sqlite_db).await?;
        }
        "REDB" => {
            let redb_db = MintRedbDatabase::new(&db_path).unwrap();
            start_cln_mint(cln_one_dir, addr, port, redb_db).await?;
        }
