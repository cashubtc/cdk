...
impl MintConnector for DirectMintConnection {
    ...
    async fn post_mint_onchain_quote(
        ...
        let res: MintQuoteOnchainResponse<Uuid> =
            self.mint.get_mint_quote(request.into()).await?.try_into()?;
        Ok(res.into())
    }
    ...
    async fn get_mint_quote_onchain_status(
        ...
        let quote_id_uuid = Uuid::from_str(quote_id).unwrap();
        let quote: MintQuoteOnchainResponse<Uuid> = self
            .mint
            .check_mint_quote(&quote_id_uuid)
            .await?
            .try_into()?;
    ...
    async fn post_melt_onchain_quote(
        ...
        let res: MeltQuoteOnchainResponse<Uuid> =
    ...
    async fn get_melt_onchain_quote_status(
        ...
        let quote_id_uuid = Uuid::from_str(quote_id).unwrap();
        let quote: MeltQuoteOnchainResponse<Uuid> = self
            .mint
            .check_melt_quote(&quote_id_uuid)
            .await?
            .try_into()?;
