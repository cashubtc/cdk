...
with lib; let
  ...
  filterNulls = set: lib.filterAttrs (n: v: v != null) set;

  filterNullsRecursive = set:
    lib.mapAttrs
      (name: value:
        if lib.isAttrs value && !lib.isDerivation value
        then filterNullsRecursive (filterNulls value)
        else value
      )
      (filterNulls set);

  configFile = toml.generate "config.toml" (filterNullsRecursive cfg.settings);
in
{
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.cdk-mintd = {
      description = "Cashu mint daemon";
      after =
        [ "network.target" ]
        ++ optional (cfg.settings.database.engine == "postgres") "postgresql.service";
      ...
      serviceConfig = {
        ...
        LoadCredential =
          let
            loadCredentials =
              lib.optionals (cfg.secrets.mnemonic != null) [ "CDK_MINTD_MNEMONIC:${cfg.secrets.mnemonic}" ]
          ...
          in
          mkIf (loadCredentials != [ ]) loadCredentials;
    ...
    assertions = [
      {
        assertion = cfg.settings.info.url != "";
        message = "services.cdk-mintd.info.url must be set";
      }
      {
        assertion = cfg.settings.lightning.backend != null;
        message = "services.cdk-mintd.lightning.backend must be set";
      }
      {
        assertion = (cfg.settings.lightning.backend == "cln") -> (cfg.settings.lightning.cln.rpc_path != "");
        message = "services.cdk-mintd.lightning.cln.rpc_path must be set when using CLN backend";
      }
      {
        assertion = (cfg.settings.lightning.backend == "lnd") -> (cfg.settings.lightning.lnd.address != "" && cfg.settings.lightning.lnd.macaroon_file != "" && cfg.settings.lightning.lnd.cert_file != "");
        message = "services.cdk-mintd.lightning.lnd.{address,macaroon_file,cert_file} must be set when using LND backend";
      }
      {
        assertion = (cfg.settings.lightning.backend == "lnbits") -> (cfg.settings.lightning.lnbits.lnbits_api != "" && (cfg.environmentFile != null || (cfg.secrets.lnbitsAdminKey != null && cfg.secrets.lnbitsInvoiceKey != "")));
      ...
      {
        assertion = (cfg.settings.database.engine == "postgres") -> (cfg.settings.database.postgres.url != "" || cfg.environmentFile != null || cfg.secrets.postgresUrl != null);
      ...
      {
        assertion = (cfg.settings.info.httpCache.backend == "redis") -> (cfg.settings.info.httpCache.keyPrefix != null && cfg.settings.info.httpCache.connectionString != null);
        message = "services.cdk-mintd.info.httpCache.{keyPrefix,connectionString} must be set when using Redis cache backend";
      }
      {
        assertion = cfg.settings.auth.enable -> (cfg.settings.auth.openidDiscovery != "" && cfg.settings.auth.openidClientId != "");
        message = "services.cdk-mintd.auth.{openidDiscovery,openidClientId} must be set when authentication is enabled";
      }
    ];

    warnings = [
      (mkIf (cfg.settings.info.mnemonic != "")
      ...
      (mkIf (cfg.settings.database.engine == "postgres" && cfg.settings.database.postgres.url != "")
