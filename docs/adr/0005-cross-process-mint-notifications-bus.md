...
* `build` also spawns the forwarding task, which opens and holds a second
  connection used for nothing but `pg_notify`, reconnecting under the same
  backoff. It is not opened during `connect`, so an instance that can listen but
  not publish starts and says so in its logs.

Delivery model:

```text
publish(event):                            // returns at once, like LocalBus
  queue event for peers                    // in the calling thread: fixes this instance's order
  spawn: local.deliver(event)              // never blocked on Postgres

forwarding task:                           // one task, one connection
  for each queued event:
    serialize { origin, event: { kind, payload } } as JSON
    SELECT pg_notify(channel, payload)     // awaited before the next event
...
Forwarding goes through one queue drained by one task because peers must see
one instance's events in the order that instance published them. NUT-17
payloads are full-state replacements, so a reordered pair presents as a state
regression (`ISSUED` before `PAID`) to a subscriber on another instance. A task
per publish, each issuing its own autocommit `pg_notify`, leaves that order to
the scheduler. The queue is written synchronously inside `publish`, which is
what fixes the order; the cost is cloning the event, while serialization moves
off the publish path onto the forwarding task. Local fan-out keeps its own task,
so a slow forward never holds in-process subscribers up.

That order is per origin. Nothing orders events published by different instances
against each other, and the two paths into a subscriber have very different
latencies: an event published on the instance the subscriber is connected to is
delivered in-process, while a peer's crosses the queue, `pg_notify`, the
database and the listener. So if instance A marks a mint quote `PAID` and the
wallet's `/mint` then lands on instance B, a subscriber on B can see B's
`ISSUED` first and A's `PAID` after it, and then sit on that stale full state
until it polls or resubscribes. Local fan-out also spawns a task per event, so
even one instance's events are not strictly ordered for its own subscribers;
only the wire is.

Cross-instance notification is therefore unordered full-state gossip whose truth
is the database. `fetch_events` at subscribe time is what resynchronizes a
subscriber, and a wallet acting on a trailing stale payload repeats an operation
that already completed rather than corrupting anything. Suppressing a payload
older than the last one delivered for a topic would need per-topic delivery
state and a monotonic order over payloads, which the events do not have: proof
states go backwards (a failed melt returns a proof from `PENDING` to `UNSPENT`)
and carry no timestamp at all.

The queue is bounded. A publish while the forwarding connection is down is
queued and flushed in order once it reconnects, so a reconnect at the backoff
floor loses nothing; past its capacity the forward is dropped and counted.
Counts are reported in one aggregated log line per episode (and on the
reconnect that ends one) rather than one line per event, so a database incident
costs bounded logging and still tells an operator how much was lost.
...
from the `fetch_events` backfill when they subscribe.
...
and are queued for peers and flushed in order once the connection comes up;
past the queue's capacity they are dropped. Peers recover the state itself only
through `fetch_events`, which runs when a subscription is created.

When mintd runs from a database-backed configuration document, the database is
opened from the bootstrap environment (`CDK_MINTD_PUBSUB_CROSS_INSTANCE`, like
`CDK_MINTD_DATABASE` and the Postgres URL) before the document can be read.
Connecting the bus is deferred past that read, so a `[database.pubsub]` block in
the stored document wins over the bootstrap value: an operator who disables the
channel there gets it disabled. The database itself is not re-opened, so the bus
still uses the bootstrap connection settings, which startup already requires the
document to agree with.
...
* Every Postgres mint holds two dedicated connections per instance, one
  listening and one publishing, each reconnected by its own task, including a
  single-instance mint that gains nothing from them beyond their cost and one
  `pg_notify` per event. They are opened directly rather than drawn from the
  pool, so they are over and above the configured `max_connections`.
...
* An outage longer than the outbound queue costs peers the live push for its
  middle. `fetch_events` runs when a subscription is created, so a wallet that
  subscribes or reconnects after the recovery sees current state; one whose
  subscription stayed open on a peer instance throughout misses those events
  until it polls or reconnects. State is never lost, because the database holds
  it, but this is weaker than "the next backfill picks it up" and the live push
  is not a delivery guarantee. Order is preserved per origin on every path,
  including a flush after a reconnect, but not across origins: a subscriber can
  be left on a full state published earlier by another instance, until it polls
  or resubscribes.
* Past that queue, the per-subscriber channel bounds what a flood can buffer,
  exactly as it does for in-process events.
