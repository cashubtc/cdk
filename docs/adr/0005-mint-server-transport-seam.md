...
`MintConnector::open_stream` defaults to unsupported, so a stream-capable
connector overrides it. `MintServer::open_stream` is required: a server
transport without streaming returns `Error::StreamingNotSupported`
explicitly, so streaming is a deliberate decision rather than a silent
omission. Implemented today:
