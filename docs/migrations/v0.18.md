...
### Review the New WebSocket Limits

The `[limits]` section gained ceilings for the `/v1/ws` endpoint: a process-wide
connection count, subscriptions and filters per connection, a per-connection
request budget, a message size cap, and the keepalive and idle timeouts. The
shipped defaults are safe for a public mint, so an import document that omits
them needs no change.

There is no per-client connection cap. A reverse proxy already sees the real
client address and is the right place to enforce that policy.
`ws_max_connections` bounds the mint as a whole.

`cdk-mintd` refuses to start on limits it could not serve, naming the key to fix:
a zero where zero means "refuse everything", a `ws_max_request_burst_units` or
`ws_max_topics_per_connection` too small to admit one
`ws_max_filters_per_subscription` request, or a `ws_ping_interval_secs` that is
not shorter than `ws_idle_timeout_secs` and so leaves a quiet client no window
to answer a ping. See `crates/cdk-mintd/example.config.toml` for every key and
its default.

### Review the New Subscription Limits

The `[limits]` section also gained ceilings for the mint's subscription
manager: `pubsub_max_topics` bounds the topic index across the whole mint,
`pubsub_max_concurrent_backfills` bounds how many subscriptions read their
initial state at once, and `pubsub_max_quote_checks_per_backfill` bounds how
many payment-backend round trips one of those reads may make.

`pubsub_backfill_timeout_secs` bounds how long one of them may hold a slot. The
check budget bounds the number of round trips, not their duration, so a stalled
payment backend could otherwise let a handful of subscriptions occupy every
slot. A backfill that runs out of time sends no initial state and logs it; the
subscription stays live and receives every later event, and a payment the
backend has already reported is still committed.

Each of these keys must be greater than zero, and `cdk-mintd` refuses to start
otherwise, naming the key to fix. Zero is not an off switch: a zero
`pubsub_max_quote_checks_per_backfill` in particular would serve every backfill
from storage without asking the payment backend, so a client could keep seeing a
quote as pending after it was paid.
