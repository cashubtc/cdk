...
## PostgreSQL Column Widths

The first v0.18 start widens the PostgreSQL columns that hold an amount, a unix
timestamp, or a value the payment backend chooses. They were declared `int4`,
which the driver truncated silently: an amount above 2.1 billion was stored
wrong, and any expiry past 2038 was stored as a date already in the past. They
are now `int8`, and the driver refuses a value a column cannot hold instead of
truncating it.

Each `ALTER TABLE` rewrites the table under an `ACCESS EXCLUSIVE` lock, so the
mint is unavailable for the length of the rewrite. Time it against a copy of
the database before upgrading a busy mint. SQLite is unaffected, its only
