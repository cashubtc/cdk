...
The `[limits]` section gained ceilings for the `/v1/ws` endpoint: connection
counts, subscriptions and filters per connection, a per-connection request
budget, a message size cap, and the keepalive and idle timeouts. The shipped
defaults are safe for a public mint, so an import document that omits them needs
no change.

One key deserves attention if you set it:
...
The cap keys on the TCP peer address, so it ships disabled: behind a reverse
proxy every connection arrives from the proxy's address, a non-zero value would
apply to all clients at once, and the mint would stop delivering NUT-17
notifications once a few connections were open. `ws_max_connections` still
bounds the process as a whole.

To cap connections per client behind a proxy, name the header the proxy sets:

```toml
[limits]
ws_max_connections_per_ip = 4
ws_trusted_client_ip_header = "X-Forwarded-For"
```

Only set the header when the proxy overwrites or appends to it. Any client can
send it otherwise, which makes the cap trivial to evade. The rightmost address
in the header is used, because that is the address the proxy saw; a request that
arrives without the header, or whose header does not end in an address, is held
to its peer address instead.
...
`ws_max_filters_per_subscription` request, or a `ws_ping_interval_secs` past
`ws_idle_timeout_secs`. See `crates/cdk-mintd/example.config.toml` for every key
and its default.
