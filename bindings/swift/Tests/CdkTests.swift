...
struct CdkTests {
    ...
    init() async throws {
        ...
        wallet = try Wallet.open(request: WalletOpenRequest(
            ...
            config: WalletConfig(advanced: nil)
        ))
    }
    ...
    func initialBalanceIsZero() async throws {
        ...
        let operations = try await wallet.operations(query: OperationQuery(
            kinds: [],
            states: [],
            limit: nil
        ))
    ...
    private func configuredWallet(_ rateLimit: RateLimit?) throws -> Wallet {
        try Wallet.open(request: WalletOpenRequest(
            ...
            mnemonic: try generateMnemonic(),
            store: .sqlite(path: ":memory:"),
            config: WalletConfig(advanced: nil, rateLimit: rateLimit)
        ))
    }
    ...
    func mintFlow() async throws {
        let session = try await wallet.requestMint(request: MintRequest(
            method: .bolt11,
        ...
        #expect(claimed.amount.value == 100)
