...
func TestWalletOpenProvidesLocalState(t *testing.T) {
	...
	operations, err := wallet.Operations(OperationQuery{})
	if err != nil {
		t.Fatalf("Operations: %v", err)
	}
	if len(operations) != 0 {
		t.Fatalf("expected no durable operations, got %d", len(operations))
	}
...
func TestWalletManagerCoordinatesMultipleMints(t *testing.T) {
	mnemonic, err := GenerateMnemonic()
	if err != nil {
		t.Fatalf("GenerateMnemonic: %v", err)
	}
	manager, err := WalletManagerOpen(WalletManagerOpenRequest{
		Mnemonic: mnemonic,
		Store:    WalletStoreSqlite{Path: tempDBPath(t)},
	})
	if err != nil {
		t.Fatalf("WalletManagerOpen: %v", err)
	}
	defer manager.Destroy()

	first, err := manager.OpenWallet(WalletIdentity{
		MintUrl: MintUrl{Url: "https://one.example.com"},
		Unit:    CurrencyUnitSat{},
	})
	if err != nil {
	...
	defer first.Destroy()
	second, err := manager.OpenWallet(WalletIdentity{
		MintUrl: MintUrl{Url: "https://two.example.com"},
		Unit:    CurrencyUnitSat{},
	})
	if err != nil {
...
func TestInvalidOperationIDReturnsStructuredError(t *testing.T) {
	...
	_, err := wallet.ResumeSend("not-an-operation-id")
	assertInvalidInput(t, err)
}
