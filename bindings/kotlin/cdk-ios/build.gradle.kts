...
iosTargets.forEach { (platform, _) ->
    ...
    tasks.register<Jar>("${platform}SourcesJar") {
        archiveBaseName.set("cdk-ios-$platform")
        archiveClassifier.set("sources")
        from(rootProject.projectDir.resolve("../../crates/cdk-ffi/src"))
    }
    tasks.register<Jar>("${platform}JavadocJar") {
        archiveBaseName.set("cdk-ios-$platform")
    ...
    publications {
        iosTargets.forEach { (platform, _) ->
            create<MavenPublication>(platform) {
                ...
                artifact(tasks.named("${platform}SourcesJar"))
                artifact(tasks.named("${platform}JavadocJar"))
            }
        }
    }
}
