...
// ---------------------------------------------------------------------------
// Split semantics (mirrors cashu-ts splitAmount)
// ---------------------------------------------------------------------------
...
describe('split semantics', () => {
  ...
  it('throws when the keyset cannot represent the remainder', () => {
    assert.throws(
      () => creator.createRandomData(5, KEYSET_ID, COARSE_KEYS),
      /Unable to split remaining amount: 5/,
    );
  });

  it('fills the remainder when a custom split is partial', () => {
    // [2, 4] covers 6 of 10; the remaining 4 is filled from the keyset.
    const outputs = creator.createRandomData(10, KEYSET_ID, KEYS, [2, 4]);
    const amounts = outputs.map((o) => o.amount).sort((a, b) => a - b);
    assert.deepEqual(amounts, [2, 4, 4]);
  });

  it('ignores zero entries in a custom split for a positive amount', () => {
    const outputs = creator.createRandomData(6, KEYSET_ID, KEYS, [2, 4, 0]);
    const amounts = outputs.map((o) => o.amount).sort((a, b) => a - b);
    assert.deepEqual(amounts, [2, 4]);
  });

  it('throws when a custom split exceeds the amount', () => {
    assert.throws(
      () => creator.createRandomData(10, KEYSET_ID, KEYS, [8, 8]),
      /Split is greater than total amount: 16 > 10/,
    );
  });

  it('throws when a custom denomination is not in the keyset', () => {
    assert.throws(
      () => creator.createRandomData(10, KEYSET_ID, KEYS, [3]),
      /Provided amount preferences do not match the amounts of the mint keyset/,
    );
  });

  it('returns explicit zero outputs for a zero-total custom split', () => {
    // Restore / NUT-08 blank outputs: amount 0 with an all-zero split.
    const seed = new Uint8Array(64).fill(42).buffer;
    const outputs = creator.createDeterministicData(0, seed, 0, KEYSET_ID, KEYS, [0, 0]);
    assert.equal(outputs.length, 2);
    for (const o of outputs) assert.equal(o.amount, 0);
  });
});
