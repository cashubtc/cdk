...
void main() {
  ...
  setUp(() {
    ...
    wallet = Wallet.open(
      request: WalletOpenRequest(
        ...
        config: WalletConfig(advanced: null),
      ),
    );
  });
  ...
  test('initial balance is zero', () async {
    ...
    final operations = await wallet.operations(
      query: OperationQuery(kinds: const [], states: const [], limit: null),
    );
  ...
  Wallet configuredWallet(RateLimit? rateLimit) => Wallet.open(
    request: WalletOpenRequest(
      ...
      mnemonic: generateMnemonic(),
      store: SqliteWalletStore(':memory:'),
      config: WalletConfig(advanced: null, rateLimit: rateLimit),
    ),
  );
  ...
  test('zero rate limit values are rejected', () {
    expect(
      () => configuredWallet(CustomRateLimit(capacity: 0, refillPerMinute: 30)),
      throwsA(isA<FfiException>()),
    );
    expect(
      () => configuredWallet(CustomRateLimit(capacity: 5, refillPerMinute: 0)),
      throwsA(isA<FfiException>()),
    );
  });
  ...
  test(
    'mint flow',
    () async {
      final session = await wallet.requestMint(
        request: MintRequest(
      ...
      try {
        ...
        expect(claimed.amount.value, equals(100));
